...
Graph.Layout.Ordered.prototype = {
    ...
    /**
     * @return true if the given edges drawn as straight lines intersect
     */
    edgesCross: function(e1,e2){
    	var xs1 = e1.source.layoutPosX;
    	var ys1 = e1.source.layoutPosY;
    	var xt1 = e1.target.layoutPosX;
    	var yt1 = e1.target.layoutPosY;
    	
    	var xs2 = e2.source.layoutPosX;
    	var ys2 = e2.source.layoutPosY;
    	var xt2 = e2.target.layoutPosX;
    	var yt2 = e2.target.layoutPosY;
		
		var a1 = (ys1 - yt1) / (xs1 - xt1);
		var a2 = (ys2 - yt2) / (xs2 - xt2);
		...
		return x>=min(xs1, xt1) && x<=max(xs1,xt1) && x>=min(xs2, xt2) && x<=max(xs2,xt2)  		
    },
    
    layoutScore : function() {
    	var intersections = 0;
    	var edges = this.graph.edges;
    	var layouter = this;
    	$.each(edges, function(i, e1) {
    		$.each(edges, function(i, e2) {
    			if (layouter.edgesCross(e1, e2))
    				intersections++;
    		});
    	});
    	intersections/=2;
    	var score = intersections / Math.pow(edges.length, 2);
    	log("of " +edges.length+" edges, " + intersections + " cross. score: " + score);
    	return score;
    },
    
    layoutX : function() {
    	this.levels = this.graph.nodes.length / 3 + 1;
    	var score=1.0;
    	var iter=0;
    	while (score > 0.1 && iter++<50) {
	    	$.each(this.graph.nodes, function(i, node) {
	    		node.layoutPosX = Math.random();
	    	});
    	
    		score = this.layoutScore();
    	}
    	// create new permutations using $.extend(true, [], nodes);
    },
